...
const prop = (text, key) => {
  ...
  return m ? dataToText(m[1]) : null;
};

const byPrefix = (serial, table) => (serial && table.prefixes[serial.slice(0, 3)]) || null;

// Returns a list of { key, label, value, vendor, rank, note } rows.
export function analyze(files, vendors) {
  ...
  const battery = battIdx < 0 ? null
    : service.slice(battIdx).match(/"Serial" = "([^"]+)"|<key>Serial<\/key>\s*<string>([^<]+)/)?.slice(1).find(Boolean) ?? null;
